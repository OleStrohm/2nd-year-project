...
class App:
    ...
    # Transcription setup as a label
        self.transcription = tk.Label(self.root, text = 'Transcript runs here', bg = 'black', fg='white', anchor = 'nw' , height = 2)
    ...
    def settings_start(self):
        ...
        self.main_menu.geometry('%dx%d+%d+%d' % (0.6 * self.width, 0.4 * self.height, 0.2 * self.width, 0.1 * self.height))
    ...
    def settings_sip(self):
        ...
        slider_pressure.bind("<ButtonRelease>" , lambda event: self.settings_update(slider_pressure.get(), self.settings.sens))
    ...
    def uppdate_transcriot(self, line):
